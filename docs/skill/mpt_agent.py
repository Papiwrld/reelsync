#!/usr/bin/env python3
"""Cross-platform installation and video generation for the ReelSync Skill."""
...
PROJECT_ARCHIVE_URL = "https://github.com/Papiwrld/reelsync/archive/refs/heads/main.zip"
DEFAULT_ROOT = Path.home() / "ReelSync"
...
def log(message: str) -> None:
    ...
    print(f"[ReelSync] {message}", flush=True)


def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Install ReelSync and generate a final video from a topic."
    )
    ...
    parser.add_argument(
        ...
        help=f"ReelSync installation directory (default: {DEFAULT_ROOT})",
    )
    parser.add_argument(
        "cli_args",
        nargs=argparse.REMAINDER,
        help="additional ReelSync CLI arguments placed after --",
    )
...
def _safe_extract(archive: zipfile.ZipFile, destination: Path) -> None:
    ...
    for member in archive.infolist():
        ...
        if target != destination and destination not in target.parents:
            raise SkillError(
                f"project archive contains an unsafe path: {member.filename}"
            )
...
def ensure_project(root: Path) -> None:
    ...
    if root.exists() and any(root.iterdir()):
        raise SkillError(
            f"installation directory exists but is not a valid project: {root}"
        )
    ...
    with tempfile.TemporaryDirectory(prefix="mpt-install-") as temp_dir_value:
        ...
        archive_path = temp_dir / "ReelSync.zip"
        request = urllib.request.Request(
            PROJECT_ARCHIVE_URL,
            headers={"User-Agent": "ReelSync-Agent-Skill"},
        )
        ...
        if len(candidates) != 1:
            raise SkillError(
                "download completed but no valid ReelSync project was found"
            )
        if root.exists():
            root.rmdir()
...
def _provider_is_ready(text: str, provider: str) -> bool:
    ...
    if provider in KEYLESS_LLM_PROVIDERS:
        return True
    if not _has_configured_value(_plain_config_value(text, f"{provider}_api_key")):
        return False
...
def report_missing_config(provider: str, missing: list[str]) -> int:
    ...
    if any(field.startswith(f"{CUSTOM_OPENAI_PROVIDER}_") for field in missing):
        print("OPENAI_COMPATIBLE_REQUIRED=API key, API base URL, model name")
    ...
    print(
        "Request only the listed values, set the environment variables, and rerun the same command."
    )
...
def report_invalid_pexels_config() -> int:
    ...
    print(f"PEXELS_API_KEY_HELP_URL={PEXELS_API_KEY_HELP_URL}")
    print(
        "All configured Pexels API keys were rejected or are unavailable. Provide a new key."
    )
    return NEEDS_INPUT_EXIT_CODE
...
def _validate_pexels_key(api_key: str) -> str:
    ...
    request = urllib.request.Request(
        ...
        headers={
            ...
            "User-Agent": "ReelSync-Agent-Skill",
        },
    )
    try:
...
def validate_pexels_config(config_path: Path, cli_args: list[str]) -> bool:
    ...
    if unknown_count:
        log(
            "Pexels keys could not be verified due to a network or service error; keeping the existing configuration"
        )
        return True

    log(
        f"Pexels key validation failed: all {rejected_count} configured keys are unusable"
    )
    return False


def result_manifest_path(root: Path) -> Path:
    return root / ".agent-logs" / "reelsync-video" / "latest-result.json"
...
def generate_video(
    ...
    log_dir = root / ".agent-logs" / "reelsync-video"
