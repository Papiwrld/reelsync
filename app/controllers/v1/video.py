...
# Lock for in-memory idempotency key deduplication
_idempotency_lock = threading.Lock()

# 认证依赖项：配置了 api_key 时自动对所有 /api/v1 路由启用 x-api-key 校验，
# 未配置时保持开放（防止默认配置直接锁死 API）。
router = new_router(dependencies=base.auth_dependencies())
...
def _parse_byte_range(
    ...
    try:
        ...
        logger.warning(
            ...
            f"range: {range_header}, file_size: {file_size}, error: {exc!s}"
        )
        raise HttpException(
            task_id=request_id,
            status_code=416,
    ...
    while True:
        ...
        for task in tasks:
            if (
                ...
                and task.get("stop_at") == stop_at
                and task.get("state") != const.TASK_STATE_FAILED
            ):
...
def create_task(
    ...
    if idempotency_key:
        ...
        if is_redis:
            # Atomic claim using Redis SET NX EX
            task_id = utils.get_uuid()
            redis_key = f"idempotency:{stop_at}:{idempotency_key}"
            claimed = sm.state._redis.set(redis_key, task_id, nx=True, ex=86400)
            if not claimed:
                # Another request won the race; fetch the existing task_id
            ...
            # We won the race; create the task and store the idempotency key
            task = {
                "task_id": task_id,
                "request_id": request_id,
                "params": body.model_dump(),
            }
            state_kwargs = {"stop_at": stop_at, "idempotency_key": idempotency_key}
            sm.state.update_task(task_id, **state_kwargs)
...
def upload_bgm_file(request: Request, file: UploadFile = File(...)):
    ...
    try:
        ...
        logger.warning(
            f"background music upload rejected: request_id={request_id}, error={exc!s}"
        )
        raise HttpException(
            task_id=request_id,
            status_code=400,
            message=f"{request_id}: {exc!s}",
        )
    except bgm_service.BgmServiceError as exc:
        # 工具链或存储故障属于服务端问题，不能伪装成用户文件错误。日志保留
        # request_id 和内部原因，HTTP 响应只返回稳定文案，避免暴露服务器路径。
        logger.error(
            f"background music upload failed: request_id={request_id}, error={exc!s}"
        )
        raise HttpException(
            task_id=request_id,
            status_code=500,
...
def upload_video_material_file(request: Request, file: UploadFile = File(...)):
    ...
    if suffix in allowed_suffixes:
        ...
        tmp_path = f"{save_path}.{utils.get_uuid()}.tmp"
        # 先写临时文件再原子替换，避免两个并发上传同名字文件时互相读到
        # 写了一半的内容（Windows 下还会因为文件占用直接失败）。
        try:
