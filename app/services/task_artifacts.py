...
def _agentic_state_file(task_id: str) -> Path:
    """Agentic planning state lives beside the script manifest in the task dir."""
    return Path(utils.task_dir(task_id)) / "agentic_state.json"


def write_agentic_state(task_id: str, payload: Mapping[str, Any]) -> None:
    """Persist the agentic GenerationState for a task (atomic write)."""
    _write_json_atomic(_agentic_state_file(task_id), payload)


def load_agentic_state(task_id: str) -> dict:
    """Read a task's agentic state; missing/corrupt files yield an empty dict.

    The state is diagnostic/planning data only — generation must never be
    blocked by it, so failures degrade silently.
    """
    try:
        target = _agentic_state_file(task_id)
        with target.open("r", encoding="utf-8") as state_file:
            payload = json.load(state_file)
        return payload if isinstance(payload, dict) else {}
    except Exception as exc:  # noqa: BLE001
        logger.debug(
            f"skip agentic state load because it is unavailable: "
