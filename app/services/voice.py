...
def generate_silent_audio(duration_seconds: float, output_file: str) -> bool:
    ...
    try:
        ...
        logger.error("failed to generate silent audio: ffmpeg timed out")
        return False
...
def azure_tts_v2(
    ...
    for i in range(3):
        try:
            ...
            # 同步 .get() 在网络异常时可能无限挂起，这里给 SDK 一个明确超时。
            result = speech_synthesizer.speak_ssml_async(ssml).get(timeout=60)
            ...
            # 失败/取消时清掉 SDK 可能已写出的空或半成品音频，避免下游把
            # 残缺文件当成有效语音，也避免重试时旧文件残留。
            try:
                if os.path.exists(voice_file) and os.path.getsize(voice_file) == 0:
                    os.remove(voice_file)
            except OSError:
            ...
            # 失败时清掉 SDK 可能已写出的空/半成品音频，避免下游把空文件
            # 当成有效语音。
            try:
                if os.path.exists(voice_file) and os.path.getsize(voice_file) == 0:
                    os.remove(voice_file)
...
def chatterbox_tts(
    ...
    payload = {
        ...
        "voice": voice,
        "response_format": "mp3",
        # OpenAI speech API accepts speed 0.25-4.0; ReelSync's rate is a
