...
# 容忍 mtime 相对当前时钟最多偏差这么远（秒）。文件系统时间戳粒度粗（如
# 网络共享、FAT 卷）或两台机器时钟轻微不一致时，刚写入的缓存文件可能显示
# mtime 在未来。只要偏差在这个范围内仍视为新鲜，避免把刚生成的缓存误判为
# “来自未来”而删除后触发一次不必要的远端搜索。真正的未来时间（大偏差）
# 仍按无效处理，防止回拨的时钟让缓存长期不失效。
_CACHE_CLOCK_SKEW_TOLERANCE_SECONDS = 30.0
...
def _cached_source_info(item: MaterialInfo) -> dict | None:
    ...
    # 文本元数据用于 auto 来源的相关性排序，这里一并进入缓存白名单。
    for field in ("title", "tags", "description", "media_type"):
        value = source.get(field)
        if isinstance(value, str) and value.strip():
            cached[field] = value.strip()
...
def load_material_search_cache(
    ...
    *,
    ...
    # 系统时间回拨或文件从其它机器复制后，mtime 可能明显落在未来。此时不能
    # 把缓存长期视为新鲜数据，直接失效并重新请求远端更可靠。但文件系统时间戳
    # 粒度或轻微时钟偏差也会让刚写入的文件显示为“未来几毫秒/秒”，因此未来
    # 偏差在容忍范围内时仍视为有效，避免误删刚生成的缓存。
    if (
        cache_age < -_CACHE_CLOCK_SKEW_TOLERANCE_SECONDS
        ...
        for raw_item in payload["items"]:
            ...
            items.append(
                MaterialInfo(
                    ...
                    resolution=(
                        raw_item.get("resolution") or ""
                        if isinstance(raw_item.get("resolution"), str)
                        else ""
                    ),
            ...
            serialized_items.append(
                {
                    ...
                    "resolution": (
                        item.resolution if isinstance(item.resolution, str) else ""
                    ),
                    "source_info": source_info,
                }
            )
    ...
    with entries:
        for entry in entries:
            ...
            try:
                ...
                if (
                    -_CACHE_CLOCK_SKEW_TOLERANCE_SECONDS
                    <= cache_age
