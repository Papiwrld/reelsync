...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Generate ReelSync videos without the WebUI.\n\n"
...
""",
    ...
    content_group.add_argument(
        ...
        type=_positive_int,
        default=None,
        help="target video length in seconds for script generation (default: follow content profile)",
    )
    content_group.add_argument(
        "--video-script-prompt",
        default=None,
    ...
    material_group.add_argument(
        "--material-media-type",
        default=None,
        choices=["images_videos", "videos_only"],
        help="images_videos (default) mixes generated still images with real clips; videos_only uses real footage only",
    )
    material_group.add_argument(
        "--image-motion-effect",
        default=None,
        choices=["kenburns", "zoom_in", "zoom_out", "slide_left", "slide_right", "fade", "random"],
        help="how still images are animated into video clips (default: kenburns)",
    )
    material_group.add_argument(
        "--video-materials",
        default="",
    ...
    video_group.add_argument(
        ...
        default=None,
        help=(
            "maximum duration of each source clip in seconds; 0 = auto derive "
            "from target duration and material count (default: 5)"
        ),
    )
    video_group.add_argument(
    ...
    video_group.add_argument(
        ...
        action=argparse.BooleanOptionalAction,
        help=(
            "render title/fact/callout overlay cards aligned to narration "
            "(default: disabled)"
        ),
    )
    video_group.add_argument(
        "--overlay-style",
        default=None,
        choices=["title_fact", "title_only", "facts_only", "callouts_only", "full"],
        help="which overlay cards to render (default: title_fact)",
    )
    video_group.add_argument(
        "--overlay-image",
        default=None,
        metavar="PATH",
        help="corner overlay image (logo/watermark) shown over the whole video",
    )
    video_group.add_argument(
    ...
    subtitle_group.add_argument(
        ...
        default=None,
        help=(
            "subtitle font filename inside resource/fonts (default: Montserrat-Bold.ttf)"
        ),
    )
    subtitle_group.add_argument(
        "--subtitle-position",
...
def build_video_params(args: argparse.Namespace) -> VideoParams:
    ...
    if args.target_duration is not None:
        params_kwargs["video_duration_seconds"] = args.target_duration
    if args.material_media_type is not None:
        params_kwargs["material_media_type"] = args.material_media_type
    if args.image_motion_effect is not None:
        params_kwargs["image_motion_effect"] = args.image_motion_effect

    if args.overlay_enabled is not None:
        params_kwargs["overlay_enabled"] = args.overlay_enabled
    if args.overlay_style is not None:
        params_kwargs["overlay_style"] = args.overlay_style
    if args.overlay_image is not None:
        params_kwargs["overlay_image"] = args.overlay_image
    if args.overlay_image_opacity is not None:
        params_kwargs["overlay_image_opacity"] = args.overlay_image_opacity
