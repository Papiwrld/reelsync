...
# 设置/ReelSync目录权限为777
RUN chmod 777 /ReelSync

ENV PYTHONPATH="/ReelSync"
...
# docker build -t reelsync .

# 2. Run the Docker container using the following command
## For Linux or MacOS:
# docker run -v $(pwd)/config.toml:/ReelSync/config.toml -v $(pwd)/storage:/ReelSync/storage -p 127.0.0.1:8501:8501 reelsync
## For Windows:
# docker run -v ${PWD}/config.toml:/ReelSync/config.toml -v ${PWD}/storage:/ReelSync/storage -p 127.0.0.1:8501:8501 reelsync
