...
class TestInMemoryTaskManager(unittest.TestCase):
    ...
    def test_check_queue_handles_dequeue_returning_none(self):
        ...
        with (
            patch.object(manager, "is_queue_empty", return_value=False),
            patch.object(manager, "dequeue", return_value=None),
...
class TestRedisTaskManager(unittest.TestCase):
    def setUp(self):
        ...
        patcher = patch(
            "app.controllers.manager.redis_manager.redis.Redis",
            return_value=self.redis_client,
        )
        self.addCleanup(patcher.stop)
        redis_cls = patcher.start()
