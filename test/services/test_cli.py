...
class TestCli(unittest.TestCase):
    ...
    def test_video_clip_duration_accepts_zero_for_auto_mode(self):
        """CLI 允许 --video-clip-duration 0，表示自动推导素材时长。"""
        args = cli.parse_args(
            ["--video-subject", "自动时长", "--video-clip-duration", "0"]
        )
        params = cli.build_video_params(args)
        self.assertEqual(params.video_clip_duration, 0)

    def test_overlay_flags_map_to_video_params(self):
        """叠加层 CLI 参数应映射到 VideoParams 对应字段。"""
        args = cli.parse_args(
            [
                "--video-subject",
                "叠加层",
        ...
        params = cli.build_video_params(args)
        self.assertTrue(params.overlay_enabled)
        self.assertEqual(params.overlay_style, "full")
        self.assertAlmostEqual(params.overlay_image_opacity, 0.7)

    def test_overlay_style_choices_reject_unknown(self):
        """未知 overlay-style 值应在参数解析阶段被拒绝。"""
        with self.assertRaises(SystemExit) as cm:
            cli.parse_args(
                ["--video-subject", "叠加层", "--overlay-style", "bogus"]
            )
        self.assertEqual(cm.exception.code, 2)
    ...
    def test_help_does_not_initialize_application_or_write_logs(self):
        ...
        self.assertIn("Generate ReelSync videos", result.stdout)
