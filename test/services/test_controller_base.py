...
class TestControllerAuthentication(unittest.TestCase):
    ...
    def test_auth_dependencies_enabled_when_api_key_configured(self):
        """配置了 api_key 时，路由必须挂载 verify_token 依赖。"""
        config.app["api_key"] = "secret"

        dependencies = base.auth_dependencies()

        self.assertIsNotNone(dependencies)
        self.assertEqual(len(dependencies), 1)
        self.assertIs(dependencies[0].dependency, base.verify_token)

    def test_auth_dependencies_disabled_without_api_key(self):
        """未配置 api_key 时保持开放，与历史行为一致，不锁死 API。"""
        config.app["api_key"] = ""

        self.assertIsNone(base.auth_dependencies())
