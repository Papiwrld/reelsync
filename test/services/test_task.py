...
class TestTaskService(unittest.TestCase):
    ...
    def test_sanitize_pipeline_error_strips_credentials(self):
        """写入任务状态的失败信息必须抹掉 URL 账号密码和敏感查询参数。"""
        dirty = (
            "download failed: "
            "https://user:supersecret@cdn.example.com/clip.mp4 "
            "(query: ?api_key=abc123&token=xyz&page=2)"
        )
        clean = tm._sanitize_pipeline_error(dirty)
    ...
    def test_generate_script_forwards_script_style(self):
        """任务生成必须把文案风格传给 LLM 服务层。"""
        params = VideoParams(
            video_subject="咖啡",
            video_script="",
            video_language="zh-CN",
        ...
        self.assertEqual(
            generate.call_args.kwargs.get("script_style"), "storytelling"
        )
    ...
    def test_generate_terms_prefers_scene_plan_search_terms(self):
        """
        Agentic 场景规划生成的具体逐场景搜索词应优先于 LLM 全局主题词：
        素材下载使用视觉导演为每个场景挑选的可视化关键词，能显著提升画面
        与文案的贴合度。场景词为空时仍回退到 LLM 关键词。
        """
        params = VideoParams(video_subject="历史事件", video_script="")
        ...
        with (
            patch.object(
                ...
                return_value={
                    "scenes": [
                        {"search_terms": ["old hospital ward", "scientist in laboratory"]},
                        {"search_terms": "vintage newspaper archives"},
                        {"search_terms": []},
                    ]
                },
            ) as load_state,
            ...
            result = tm.generate_terms("task-id", params, "一些文案")

        self.assertEqual(
            result,
            ["old hospital ward", "scientist in laboratory", "vintage newspaper archives"],
        )
        load_state.assert_called_once_with("task-id")
        generate.assert_not_called()

    def test_generate_terms_falls_back_to_llm_when_no_scene_plan(self):
        """没有 Agentic 场景词时，行为与原来完全一致：调用 LLM 生成关键词。"""
        params = VideoParams(video_subject="城市通勤", video_script="")

        with (
            patch.object(tm.task_artifacts, "load_agentic_state", return_value={}),
            patch.object(
                tm.llm, "generate_terms", return_value=["city"]
            ) as generate,
        ):
            result = tm.generate_terms("task-id", params, "先城市")

        self.assertEqual(result, ["city"])
        generate.assert_called_once()

    def test_resolve_auto_clip_duration_uses_video_duration_seconds(self):
        """video_clip_duration=0 时按 video_duration_seconds / 素材数推导。"""
        params = VideoParams(
            video_subject="主题", video_clip_duration=0, video_duration_seconds=60
        )
        self.assertEqual(tm.resolve_auto_clip_duration(params, term_count=6), 10)

    def test_resolve_auto_clip_duration_prefers_video_duration_over_audio(self):
        """video_duration_seconds 存在时优先使用它，而不是音频时长。"""
        params = VideoParams(
            video_subject="主题", video_clip_duration=0, video_duration_seconds=30
        )
        self.assertEqual(
            tm.resolve_auto_clip_duration(params, term_count=10, audio_duration=120),
            3,
        )

    def test_resolve_auto_clip_duration_falls_back_to_audio_duration(self):
        """两个时长字段都为 0 时用音频时长兜底。"""
        params = VideoParams(video_subject="主题", video_clip_duration=0)
        self.assertEqual(tm.resolve_auto_clip_duration(params, term_count=3, audio_duration=45), 12)

    def test_resolve_auto_clip_duration_clamps_lower_bound(self):
        """推导结果不会低于 2 秒，保证每段素材有基本可视时长。"""
        params = VideoParams(
            video_subject="主题", video_clip_duration=0, video_duration_seconds=10
        )
        self.assertEqual(tm.resolve_auto_clip_duration(params, term_count=20), 2)

    def test_resolve_auto_clip_duration_keeps_explicit_value(self):
        """用户显式指定时长时原样返回，不做自动推导。"""
        params = VideoParams(video_subject="主题", video_clip_duration=7)
        self.assertEqual(tm.resolve_auto_clip_duration(params, term_count=3), 7)

    def test_resolve_auto_clip_duration_defaults_when_no_signals(self):
        """完全没有时长与素材信号时回到安全的 5 秒默认值。"""
        params = VideoParams(video_subject="主题", video_clip_duration=0)
        self.assertEqual(tm.resolve_auto_clip_duration(params, term_count=0), 5)

    def test_resolve_auto_clip_duration_writes_back_on_params(self):
        """自动模式解析后把结果回写到 params，下游共用同一份时长。"""
        params = VideoParams(
            video_subject="主题", video_clip_duration=0, video_duration_seconds=120
        )
        resolved = tm.resolve_auto_clip_duration(params, term_count=15)
        params.video_clip_duration = resolved
        self.assertEqual(params.video_clip_duration, 8)
    ...
    def test_recover_interrupted_generation_tasks_marks_stuck_processing_failed(self):
        """进程重启后 Redis 中残留的 PROCESSING 生成任务必须收敛为失败。"""
        state = MemoryState()
        state.update_task("stuck-task", state=tm.const.TASK_STATE_PROCESSING, progress=40)
        state.update_task(
            "done-task", state=tm.const.TASK_STATE_COMPLETE, progress=100
        )

        with patch.object(tm.sm, "state", state):
            recovered = tm.recover_interrupted_generation_tasks(page_size=10)

        self.assertEqual(recovered, 1)
        stuck = state.get_task("stuck-task")
        self.assertEqual(stuck["state"], tm.const.TASK_STATE_FAILED)
        self.assertEqual(stuck["failed_stage"], "restart")
        self.assertEqual(stuck["error"], tm._INTERRUPTED_GENERATION_ERROR)
        done = state.get_task("done-task")
        self.assertEqual(done["state"], tm.const.TASK_STATE_COMPLETE)
