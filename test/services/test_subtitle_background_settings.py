...
class TestSubtitlePresets(unittest.TestCase):
    def test_registry_contains_all_builtin_presets(self):
        ...
        for expected in ("hormozi", "tiktok", "capcut", "cinematic", "minimal", "neon"):
            self.assertIn(expected, keys)
        self.assertIn("custom", keys)

    def test_tiktok_preset_resolves_viral_look(self):
        """TikTok 预设：大写、黄色逐词高亮、pop-bounce、动态定位。"""
        from app.models.schema import SubtitlePreset
        ...
        params = VideoParams(video_subject="x", subtitle_style_preset=SubtitlePreset.TIKTOK)
        config = SubtitleStyleResolver(font_registry=get_font_registry()).resolve(params)
        self.assertEqual(config.casing.value, "uppercase")
        self.assertEqual(config.highlight_color, "#FFD60A")
        self.assertEqual(config.outline_width, 3.5)
        self.assertTrue(config.pop_bounce)
        self.assertTrue(config.active_word_highlight)
        self.assertTrue(config.dynamic_scaling)
        self.assertEqual(config.animation.value, "pop_bounce")
        self.assertEqual(config.font_name, "BeVietnamPro-Bold.ttf")

    def test_capcut_preset_resolves_clean_look(self):
        """CapCut 预设：原文大小写、圆角底板、黄色逐词高亮、底部定位。"""
        from app.models.schema import SubtitlePreset
        from app.services.subtitle_engine.fonts import get_font_registry
        from app.services.subtitle_engine.styles import SubtitleStyleResolver

        params = VideoParams(video_subject="x", subtitle_style_preset=SubtitlePreset.CAPCUT)
        config = SubtitleStyleResolver(font_registry=get_font_registry()).resolve(params)
        self.assertEqual(config.casing.value, "as_spoken")
        self.assertEqual(config.background, "#000000")
        self.assertTrue(config.rounded_background)
        self.assertTrue(config.pop_bounce)
        self.assertTrue(config.active_word_highlight)
        self.assertEqual(config.position.value, "bottom")
        self.assertEqual(config.font_name, "Roboto-Bold.ttf")

    def test_all_locales_include_tiktok_capcut_preset_labels(self):
        """WebUI 新增 TikTok/CapCut 预设后，所有语言必须包含对应翻译 key。"""
        i18n_dir = Path(__file__).parent.parent.parent / "webui" / "i18n"
        required_keys = {"Subtitle Preset TikTok", "Subtitle Preset CapCut"}
        for locale_file in i18n_dir.glob("*.json"):
            with self.subTest(locale=locale_file.name):
                data = json.loads(locale_file.read_text(encoding="utf-8"))
                translations = data.get("Translation", {})
