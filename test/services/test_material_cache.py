...
class TestMaterialSearchCache(unittest.TestCase):
    ...
    def test_near_future_mtime_still_counts_as_fresh(self):
        """文件系统时间戳粒度或轻微时钟偏差会让刚写入的缓存 mtime 在未来几
        秒内。只要偏差在容忍范围内就必须视为有效，否则每次写入都会触发一次
        多余的远端搜索（曾经导致缓存命中测试偶发失败）。"""
        material_cache.save_material_search_cache(
            provider="pixabay",
            search_term="nature",
            minimum_duration=5,
        ...
        cache_path = self._cache_path()
        now = 2_000_000_000.0
        skew_mtime = now - 5  # mtime 在未来 5 秒（容忍范围内）
        os.utime(cache_path, (skew_mtime, skew_mtime))
