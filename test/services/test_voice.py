...
class TestVoiceService(unittest.TestCase):
    ...
    def test_generate_subtitle_keeps_edge_provider_for_gemini_legacy_submaker(self):
        ...
        with (
            ...
            # 2~4 词分句后，整句会被拆成多个自然短语帧，但所有词必须按顺序
            # 完整保留，不能丢失或打乱（6 词句 → 3+3，3 词句保持原样）。
            self.assertIn("Gemini subtitle generation", subtitle_content)
            self.assertIn("should work now", subtitle_content)
            self.assertIn("Testing multiple lines", subtitle_content)
            frame_texts = [
                line.strip()
            ...
            self.assertEqual(
                " ".join(" ".join(frame_texts).split()),
                "Gemini subtitle generation should work now Testing multiple lines",
            )
            self.assertTrue(
                all(2 <= len(text.split()) <= 4 for text in frame_texts),
                f"frames should hold 2-4 words, got: {frame_texts}",
            )
