...
class TestSecrets(unittest.TestCase):
    def test_get_secret_env_var_takes_precedence_over_keyring(self):
        with (
            ...
            patch.dict("os.environ", {"MPT_TEST_SECRET": "env-value"}, clear=True),
        ):
    ...
    def test_get_secret_empty_env_var_falls_through_to_keyring(self):
        with (
            patch.object(secrets, "_keyring_available", return_value=True),
            patch.object(secrets._keyring, "get_password", return_value="keyring-value") as get_pw,
            patch.dict("os.environ", {"MPT_TEST_SECRET": ""}, clear=True),
        ):
            self.assertEqual(secrets.get_secret("MPT_TEST_SECRET"), "keyring-value")
        get_pw.assert_called_once_with("reelsync", "MPT_TEST_SECRET")
    ...
    def test_delete_secret_removes_from_keyring(self):
        with (
            patch.object(secrets, "_keyring_available", return_value=True),
            patch.object(secrets._keyring, "delete_password") as del_pw,
        ):
            self.assertTrue(secrets.delete_secret("MPT_TEST_SECRET"))
        del_pw.assert_called_once_with("reelsync", "MPT_TEST_SECRET")

    def test_delete_secret_missing_entry_returns_true(self):
        with (
            patch.object(secrets, "_keyring_available", return_value=True),
            patch.object(
                secrets._keyring.errors,
                "PasswordDeleteError",
                type("PasswordDeleteError", (Exception,), {}),
            ),
        ):
            with patch.object(
                secrets._keyring,
                "delete_password",
                side_effect=secrets._keyring.errors.PasswordDeleteError(),
            ):
                self.assertTrue(secrets.delete_secret("MPT_TEST_SECRET"))

    def test_delete_secret_returns_false_when_keyring_unavailable(self):
        with patch.object(secrets, "_keyring_available", return_value=False):
            self.assertFalse(secrets.delete_secret("MPT_TEST_SECRET"))

    def test_has_keyring_true_with_real_backend(self):
        with patch.object(secrets, "_keyring_available", return_value=True):
            self.assertTrue(secrets.has_keyring())

    def test_has_keyring_false_when_unavailable(self):
        with patch.object(secrets, "_keyring_available", return_value=False):
            self.assertFalse(secrets.has_keyring())
