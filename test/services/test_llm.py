...
class TestFallbackApiKeys(unittest.TestCase):
    """多 Key 故障转移：主 Key 失败时自动尝试备用 Key。"""

    def test_collect_llm_api_keys_orders_primary_first(self):
        runtime = {
            "moonshot_api_key": "primary-key",
            "llm_fallback_api_keys": "fallback-1, fallback-2",
        }
        provider = get_llm_provider("moonshot")
        keys = llm._collect_llm_api_keys(runtime, provider)
        self.assertEqual(keys, ["primary-key", "fallback-1", "fallback-2"])

    def test_collect_llm_api_keys_handles_newline_and_list(self):
        provider = get_llm_provider("moonshot")
        runtime = {
            "moonshot_api_key": "primary",
            "llm_fallback_api_keys": "fallback-1\nfallback-2",
        }
        self.assertEqual(
            llm._collect_llm_api_keys(runtime, provider),
            ["primary", "fallback-1", "fallback-2"],
        )
        runtime_list = {
            "moonshot_api_key": "primary",
            "llm_fallback_api_keys": ["fb1", "fb2"],
        }
        self.assertEqual(
            llm._collect_llm_api_keys(runtime_list, provider),
            ["primary", "fb1", "fb2"],
        )

    def test_collect_llm_api_keys_empty_falls_back_to_single_empty(self):
        provider = get_llm_provider("moonshot")
        self.assertEqual(llm._collect_llm_api_keys({}, provider), [""])

    def test_generate_response_tries_fallback_key_when_primary_fails(self):
        ...
        def fake_generate_with_key(prompt, runtime_app_config, llm_provider, provider, api_key):
            ...
            return f"generated with {api_key}"

        with (
            patch.object(
                llm,
                "_generate_response_with_key",
                side_effect=fake_generate_with_key,
            ),
            patch.object(config, "app", {"llm_provider": "moonshot", "moonshot_api_key": "primary-key", "llm_fallback_api_keys": "backup-key"}),
        ):
            result = llm._generate_response("hello")
        self.assertEqual(result, "generated with backup-key")
        self.assertEqual(calls, ["primary-key", "backup-key"])
    ...
    def test_generate_response_returns_error_when_all_keys_fail(self):
        ...
        self.assertIn("backup-key", result)

    def test_generate_response_success_on_primary(self):
        ...
        self.assertEqual(calls, ["primary-key"])


class TestScriptStyle(unittest.TestCase):
    """文案措辞风格预设与提示词注入。"""

    def test_default_style_is_simple_humanized(self):
        self.assertEqual(llm.DEFAULT_SCRIPT_STYLE, "simple_humanized")
        self.assertIn("simple", llm.resolve_script_style(""))

    def test_unknown_style_falls_back_to_default(self):
        self.assertEqual(
            llm.resolve_script_style("nonexistent-style"),
            llm.SCRIPT_STYLE_PRESETS["simple_humanized"],
        )

    def test_all_presets_resolve_to_instructions(self):
        for style in llm.SCRIPT_STYLE_PRESETS:
            instruction = llm.resolve_script_style(style)
            self.assertTrue(instruction.strip())
            self.assertGreater(len(instruction), 20)

    def test_build_script_prompt_injects_style(self):
        prompt = llm.build_script_prompt(
            video_subject="AI", script_style="field_expert"
        )
        self.assertIn("# Writing Style:", prompt)
        self.assertIn("field expert", prompt.lower())

    def test_build_script_prompt_default_style(self):
        prompt = llm.build_script_prompt(video_subject="AI")
        self.assertIn("# Writing Style:", prompt)
        self.assertIn("conversational", prompt.lower())

    def test_generate_script_passes_style_to_prompt(self):
        """generate_script 必须把风格传入 build_script_prompt。"""
        with (
            patch.object(
                llm,
                "build_script_prompt",
                wraps=llm.build_script_prompt,
            ) as wrapped,
            patch.object(llm, "_generate_response", return_value="final script"),
        ):
            llm.generate_script(
                video_subject="AI",
        ...
        self.assertEqual(kwargs.get("script_style"), "storytelling")
