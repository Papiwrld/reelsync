...
class TestVideoControllerTasks(unittest.TestCase):
    ...
    def test_create_task_queues_requested_pipeline_stage(self):
        ...
        update_task.assert_called_once_with("task-123", stop_at="audio")
    ...
    def test_create_task_reuses_existing_task_for_idempotency_key(self):
        """
        客户端带 X-Idempotency-Key 重试时，应复用同一阶段、未失败的任务，
        而不是重复入队（重复计费 / 重复占用磁盘）。
        """
        body = MagicMock()
        body.model_dump.return_value = {"video_subject": "Coffee"}
        ...
        with (
            patch.object(video_controller.sm.state, "get_all_tasks", return_value=([], 0)),
            patch.object(video_controller.utils, "get_uuid", return_value="task-456"),
            patch.object(video_controller.sm.state, "update_task") as update_task,
            patch.object(video_controller.task_manager, "add_task") as add_task,
        ):
            request = SimpleNamespace(
                headers={
                    ...
                    "X-Idempotency-Key": "gen-video-001",
                }
            )
            response = video_controller.create_task(request, body, stop_at="video")

        self.assertEqual(response["status"], 200)
        self.assertEqual(response["data"]["task_id"], "task-456")
        update_task.assert_called_once_with(
            "task-456", stop_at="video", idempotency_key="gen-video-001"
        )
        add_task.assert_called_once_with(
            video_controller.tm.start,
            task_id="task-456",
    ...
    def test_create_task_returns_existing_id_for_repeated_idempotency_key(self):
        """
        相同的幂等键 + 阶段再次提交时直接返回已有任务 ID，不再创建新任务。
        """
        body = MagicMock()
        body.model_dump.return_value = {"video_subject": "Coffee"}
        ...
        with (
            ...
            patch.object(video_controller.utils, "get_uuid") as get_uuid,
            patch.object(video_controller.sm.state, "update_task") as update_task,
            patch.object(video_controller.task_manager, "add_task") as add_task,
        ):
            request = SimpleNamespace(
        ...
        self.assertEqual(response["data"]["task_id"], "existing-task")
        get_uuid.assert_not_called()
        update_task.assert_not_called()
        add_task.assert_not_called()

    def test_create_task_idempotency_key_race_condition_in_memory(self):
        """
        并发请求携带相同 X-Idempotency-Key 时，内存模式下只应创建一个任务。
        使用线程锁 + 双重检查防止竞态。
        """
        import threading
        import time
        ...
        # 使用真实的内存状态而非 mock，以验证锁的实际行为
        from app.services.state import MemoryState
        original_state = video_controller.sm.state
        video_controller.sm.state = MemoryState()
        ...
        try:
            ...
            def create_task_thread():
                try:
                    request = SimpleNamespace(
                        headers={
                            "x-task-id": f"request-{threading.current_thread().ident}",
                            "X-Idempotency-Key": "race-key-001",
                        }
                    )
                    with (
                        patch.object(video_controller.task_manager, "add_task") as add_task,
                        ...
                        # 让每个线程生成不同的 UUID，但只有第一个应该成功
                        get_uuid.side_effect = lambda: f"task-{threading.current_thread().ident}"
                        response = video_controller.create_task(request, body, stop_at="video")
                        created_task_ids.append(response["data"]["task_id"])
                except Exception as e:
                    errors.append(e)

            threads = [threading.Thread(target=create_task_thread) for _ in range(10)]
            for t in threads:
                t.start()
            for t in threads:
                t.join()
            ...
            # 只应创建一个任务
            self.assertEqual(len(set(created_task_ids)), 1, f"Multiple tasks created: {created_task_ids}")
            self.assertEqual(len(created_task_ids), 10, "All 10 requests should return the same task_id")
        finally:
            video_controller.sm.state = original_state
